...
import { processAudio, exportAudio, ManualProcessingParams } from "@/utils/audioProcessing";
...
const Index = () => {
  ...
  const [manualParams, setManualParams] = useState<ManualProcessingParams | null>(null);
  const [currentTime, setCurrentTime] = useState(0);
  const [isPlaying, setIsPlaying] = useState(false);
  const [isProcessing, setIsProcessing] = useState(false);
  const [isDownloading, setIsDownloading] = useState<{ [format: string]: boolean }>({
    wav: false,
    mp3: false,
    mp4: false
  });
  ...
  const handleManualParametersChange = useCallback(async (params: ManualProcessingParams) => {
  ...
  const saveProcessedAudio = async (audio: AudioBuffer, preset: string) => {
    ...
    try {
      ...
      const originalFilePath = `${user.id}/${Date.now()}_original_${originalFileName}.wav`;
      ...
      const enhancedFilePath = `${user.id}/${Date.now()}_enhanced_${enhancedFileName}.wav`;
      
      await supabase.storage
        .from('audio')
        .upload(enhancedFilePath, blob);
  ...
  const handleDownload = useCallback(async (format: 'mp3' | 'wav' | 'mp4' = 'wav') => {
    ...
    const presetSuffix = activePreset ? `-${activePreset}` : manualParams ? '-custom' : '';
  ...
  }, [processedAudioBuffer, audioBuffer, originalFileName, originalFile, activePreset, manualParams, toast]);
      ...
      <header className="container mx-auto py-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Professional audio enhancement made simple</p>
          </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <Button 
                      ...
                      disabled={(!processedAudioBuffer && !audioBuffer) || isDownloading.wav || isProcessing || isSaving}
                      className="w-full"
                    >
                      {isDownloading.wav ? (
                        <>
                    ...
                    <Button 
                      ...
                      disabled={(!processedAudioBuffer && !audioBuffer) || isDownloading.mp3 || isProcessing || isSaving}
                      variant="outline"
                      className="w-full"
                    >
                      {isDownloading.mp3 ? (
                    ...
                    {isVideoFile && (
                      <Button 
                        ...
                        disabled={(!processedAudioBuffer && !audioBuffer) || !isVideoFile || isDownloading.mp4 || isProcessing || isSaving}
                        variant="outline"
                        className="w-full"
                      >
                        {isDownloading.mp4 ? (
                  ...
                  <p className="text-xs text-muted-foreground">
                    {isProcessing ? "Processing audio..." : isSaving ? "Saving to your library..." : "Download with applied effects"}
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-6 space-y-4">
                ...
                <p className="text-sm text-muted-foreground">
                  Enhance your audio files with professional-grade processing tools.
                  Select from our range of presets designed for different audio types.
                </p>
      ...
      <footer className="border-t bg-card">
        <div className="container mx-auto py-4 px-6 text-center text-sm text-muted-foreground">
          ...
          <p className="mt-2 text-xs">
            Keyboard shortcuts: <span className="font-mono bg-secondary px-1 rounded">Space</span> Play/Pause, 
            <span className="font-mono bg-secondary px-1 rounded ml-1">R</span> Restart, 
            <span className="font-mono bg-secondary px-1 rounded ml-1">Ctrl+D</span> Download
          </p>
        </div>
      </footer>
